    ...
    def resume_training(self, resume_state):
        ...
        if self.opt['is_train'] and self.opt.get('use_amp', False):
            if resume_state.get('amp_scaler', None):
                ...
                # 更新调度器的状态以匹配新的参数
        for scheduler in self.schedulers:
            if isinstance(scheduler, lr_scheduler.CosineAnnealingRestartCyclicLR):
                # 更新调度器的参数
                scheduler.periods = self.opt['train']['scheduler']['periods']
                scheduler.restart_weights = self.opt['train']['scheduler']['restart_weights']
                scheduler.eta_mins = self.opt['train']['scheduler']['eta_mins']
                scheduler.cumulative_period = [
                    sum(scheduler.periods[0:i + 1]) for i in range(len(scheduler.periods))
                ]
