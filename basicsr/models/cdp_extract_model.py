...
class CdpExtractModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(CdpExtractModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        self.color_loss = color_loss(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            ...
            lr, hr = self.lq, self.gt  # low-resolution input and high-resolution ground truth
            # 根据当前迭代次数选择输入
            input_data = (lr, hr)
            # 调用网络的前向传播
            data_dict = self.net_g(input_data)

            self.output = data_dict['enhancement_image']
            ...
            l_color_loss = self.color_loss(self.output, self.gt)
            l_process_img_loss = self.smooth_cri_pix(process_img, process_gt_img)
    
            loss = (l_enhancement_loss + 
                    l_process_img_loss +
                    0.1 * l_color_loss + 
                    0.015 * l_perceptual_loss  )       
            # 记录损失，l_perceptual_loss数量级在1e0-1e-1之间，l_process_img_loss和l_enhancement_loss相近，在1e-2-1e-3之间，l_color_loss在1e-1-1e-2之间，较大。
            # 所以在上次的实验中，出现了PSNR对应的值较低，但是lpips对应的值表现最好，和大权重的l_perceptual_loss有关
            # 因此以l_process_img_loss和l_enhancement_loss和l_perceptual_loss作为大权重，l_color_loss作为小权重
            # 训练慢的原因也包括使用l_perceptual_loss每次计算时间的花费较大
            ...
            loss_dict['l_color_loss'] = l_color_loss.detach()
        ...
        # 使用AMP的scaler来更新优化器的参数,包括模型参数=self.optimizer_g.step()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        with_gt_mean = self.opt['val'].get('use_gt_mean') is not None
        if with_metrics:  # 获得多种评价名字，比并初始化0
            self.metric_results = {
                metric: 0
        ...
        #import pdb
         # Pause the program execution here,Execution will pause here and you can interact with the code
